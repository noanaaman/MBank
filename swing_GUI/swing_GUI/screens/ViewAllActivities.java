...
public class ViewAllActivities extends JFrame {
	...
	public ViewAllActivities(MainScreen mainScreen) throws MBankException {
		...
		ArrayList<Activity> activities = adminAction.viewAllActivities();
		if (activities != null) {
			final String[][] allData = new String[activities.size()][5];
			for (int i = 0; i < activities.size(); i++) {
				String activityId = Long.toString(activities.get(i).getId());
				String amount = Double.toString(activities.get(i).getAmount());
				String activityDate = new SimpleDateFormat("yyyy-MM-dd").format(activities.get(i).getActivity_date().getTime());
				String commission = Double.toString(activities.get(i).getCommission());
	        ...
	        bp.add(jbCancel);
			JLabel head = new JLabel("All MBank activities:");
			...
			JOptionPane.showMessageDialog(this, "There are no recorded activities.", "No Activities", JOptionPane.INFORMATION_MESSAGE);
		}
